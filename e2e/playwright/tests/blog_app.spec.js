...
describe('when there are multiple users', () => {
  ...
  test('any registered users can see the like button', async({ page }) => {
    // Confirm the like button is visible
    const blog = page.getByRole('table').filter({ hasText: 'New blog by Playwright' })
    await blog.getByRole('button', { name: 'show' }).click()
    await expect(blog.getByRole('button', { name: 'like' })).toBeAttached()

    // Login as another user to check if the button is still available
    await page.getByRole('button', { name: 'logout' }).click()
    await loginWith(page, 'admin@email.com', 'password')
    await expect(blog.getByRole('button', { name: 'like' })).toBeAttached()

    // Confirm a non-logged user cannot see the button
    await page.getByRole('button', { name: 'logout' }).click()
    await expect(blog.getByRole('button', { name: 'like' })).not.toBeAttached()
  })
})
